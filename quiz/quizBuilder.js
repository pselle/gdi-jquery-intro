...
var defaultOptions = {
    ...
    saveProgress : false,           // Store responses in a cookie so user can come back later.
    takeOnlyOnce : false,           // Set a cookie to try to prevent re-taking the quiz.
    generateNonce : true,           // Generate a code on quiz completion for use in class registration.
...
var quizInit = function(xmlPath, target, options) {
    ...
    $.ajax({
        ...
        success: function(xml){
                buildQuiz(xml, target, options);
            }
    });
};

var buildQuiz = function(xml, target, options) {
...
var buildTitleHtml = function(xml) {
    ...
    titleHtml += '<input class="button" type="button" onclick="Reveal.navigateRight();" value="Begin" />';
    titleHtml += '</section>';
    return titleHtml;
};
...
var buildResultsHtml = function(xml, options) {
    ...
    if (options.generateNonce) {
        resultsHtml += '<p class="nonce">will need this code for your class registration.</p><h4 id="nonce"></h4>';
    }
    resultsHtml += '</section>';
    return resultsHtml;
};
...
var handleScoreTotal = function(scores, options) {
    ...
    if (scoreInfo) {
        // Update results slide with colors, values, messaging, and coded score.
        var resultsSlide = $('.quizResults');
        $(resultsSlide).find('#results').addClass(scoreInfo.class).find('#descrip').append(scoreInfo.descrip);
        if (options.showScore) {
...
var decodeNonce = function(nonce) {
    var nonceParts = nonce.split('Q');
    var decodedScore = Math.ceil(nonceParts[0] / .65 / nonceParts[1]) - 5;
